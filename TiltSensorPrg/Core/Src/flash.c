...
void FlashWriteAll(void) {
	...
	while (flash_ok != HAL_OK)
		flash_ok = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,
				START_ADDR_USER_PAGE + FLASH_ADDR_AccXZero,
				(uint64_t) AccXZero);

	flash_ok = HAL_ERROR;
	while (flash_ok != HAL_OK)
		flash_ok = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,
				START_ADDR_USER_PAGE + FLASH_ADDR_AccYZero,
				(uint64_t) AccYZero);

	flash_ok = HAL_ERROR;
	while (flash_ok != HAL_OK)
		flash_ok = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,
				START_ADDR_USER_PAGE + FLASH_ADDR_AccZZero,
				(uint64_t) AccZZero);

	flash_ok = HAL_ERROR;
	while (flash_ok != HAL_OK)
		flash_ok = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,
				START_ADDR_USER_PAGE + FLASH_ADDR_GyroXZero,
				(uint64_t) GyroXZero);

	flash_ok = HAL_ERROR;
	while (flash_ok != HAL_OK)
		flash_ok = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,
				START_ADDR_USER_PAGE + FLASH_ADDR_GyroYZero,
				(uint64_t) GyroYZero);

	flash_ok = HAL_ERROR;
	while (flash_ok != HAL_OK)
		flash_ok = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,
				START_ADDR_USER_PAGE + FLASH_ADDR_GyroZZero,
				(uint64_t) GyroZZero);

	flash_ok = HAL_ERROR;
	while (flash_ok != HAL_OK)
		flash_ok = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,
				START_ADDR_USER_PAGE + FLASH_ADDR_ThisAddr,
				(uint64_t) ThisAddr);

	flash_ok = HAL_ERROR;
	while (flash_ok != HAL_OK)
		flash_ok = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,
				START_ADDR_USER_PAGE + FLASH_ADDR_KGyro,
				(uint64_t) (*((uint32_t*) &KGyro)));
